                ...
                xl:max-w-6xl 2xl:max-w-7xl h-[70px] sm:h-[60px] xl:h-[70px]'>
                    ...
                    <div className='flex gap-x-[6px] sm:gap-x-2 xl:gap-x-3 '>
                        ...
                        <div className='relative flex flex-col items-center justify-center md:py-1 w-[100px] xl:w-[120px] sm:w-[80px] gap-x-2 sm:gap-x-1 cursor-pointer border-2 border-white rounded-md'
                            ...
                            <div className='flex items-center justify-center xl:gap-x-2 w-[90px] xl:w-[120px] sm:w-[80px] px-[2px] sm:px-[2px] cursor-pointer '>
                                <img className='w-7 sm:w-6 mr-1 cursor-pointer border-2 border-white rounded-full' src={selectedFlag} alt="" />
                                <p className=' xl:text-[17px] text-[14px] sm:text-[12px] font-normal sm:font-medium text-white'>
                                    {t(selectedFlag === flagLaos ? 'language.laos' : 'language.english')}
                                </p>
                            </div>
                            {isDropdownFlag && (
                                <div className='bg-[#003049] absolute w-[110px] xl:w-[120px] xl:gap-x-2 sm:w-[120px] py-2 md:p-0 md:px-3 flex flex-col items-center gap-2 md:gap-0 h-fit top-[55px] sm:top-11 left-[50%] -translate-x-1/2 border-2 border-white rounded-md'>
                                    <div className='flex items-center justify-center py-[4px]  xl:w-[100px] xl:gap-x-2 w-[90px] md:py-2 cursor-pointer border-b border-white'
                                        onClick={() => { changeLanguage('la'); setIsDropdownFlag(false); }}>
                                        <img className='w-7 sm:w-6 mr-1 cursor-pointer border-2 border-white rounded-full' src={flagLaos} alt="" />
                                        <p className=' text-[14px] sm:text-[14px] font-semibold sm:font-medium text-white'>{t('language.laos')}</p>
                                    </div>
                                    <div className='flex items-center justify-center xl:w-[100px] xl:gap-x-2 py-[4px] w-[90px] md:py-2 cursor-pointer '
                                        onClick={() => { changeLanguage('en'); setIsDropdownFlag(false); }}>
                                        <img className='w-7 sm:w-6 mr-1 cursor-pointer border-2 border-white rounded-full' src={flagUs} alt="" />
                                        <p className=' text-[14px] sm:text-[14px] font-semibold text-white'>{t('language.english')}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        <div className=' cursor-pointer ml-2 sm:hidden text-[34px] flex items-center justify-center text-white' onClick={toggleMenu}>
