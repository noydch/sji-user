...
export const VideoExample = () => {
    ...
    useEffect(() => {
        ...
        loadScript().then(setupPlayer).catch((error) => {
    ...
    const changeQuality = (qualityLabel) => {
        if (playerInstance) {
            const currentQuality = playerInstance.getPlaylistItem().sources.find(source => source.label === qualityLabel);
            if (currentQuality) {
                playerInstance.setCurrentQuality(playerInstance.getPlaylistItem().sources.indexOf(currentQuality));
            }
        }
    };
